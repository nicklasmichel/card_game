    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                self.handle_log_scroll(-event.y * self.scale_ui(36))
            elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                direction = -self.scale_ui(36) if event.button == 4 else self.scale_ui(36)
...
def handle_mouse_motion(self, position: tuple[int, int]) -> None:
    ...
    if abs(dx) > self.scale_ui(8) or abs(dy) > self.scale_ui(8):
